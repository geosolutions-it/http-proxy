...
public class HTTPProxy extends HttpServlet {
	...
	private void executeProxyRequest(HttpMethod httpMethodProxyRequest,
        ...
        for(Header header : headerArrayResponse) {
        	// Skip GZIP Responses
			if (header.getName().equalsIgnoreCase(HTTP_HEADER_ACCEPT_ENCODING) && header.getValue().toLowerCase().contains("gzip"))
				continue;
			else if (header.getName().equalsIgnoreCase(HTTP_HEADER_CONTENT_ENCODING) && header.getValue().toLowerCase().contains("gzip"))
				continue;
     ...
     */
	@SuppressWarnings("rawtypes")
	private void setProxyRequestHeaders(URL url, HttpServletRequest httpServletRequest, HttpMethod httpMethodProxyRequest) {
		...
		while(enumerationOfHeaderNames.hasMoreElements()) {
			...
			while(enumerationOfHeaderValues.hasMoreElements()) {
				...
				// Skip GZIP Responses
				if (stringHeaderName.equalsIgnoreCase(HTTP_HEADER_ACCEPT_ENCODING) && stringHeaderValue.toLowerCase().contains("gzip"))
					continue;
				if (stringHeaderName.equalsIgnoreCase(HTTP_HEADER_CONTENT_ENCODING) && stringHeaderValue.toLowerCase().contains("gzip"))
					continue;
